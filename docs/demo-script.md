...
| 0:40-1:02 | Station 01 Floor: three live Infinite packs plus the sold-out previous-pack showcase, live edges, the real refresh timestamp on the shelf badge. Open Station 02 X-Ray: EV vs cost, distribution histogram, the "what is loaded" three-band model (Chase real / Mid + Common labeled filler), per-card LIVE and ASSUMED tags. Optionally hit `/vault` for the full priced library. | EV transparency: real Index prices, a labeled model where only the Chase band is real, honest house-edge spread, every number traceable. |
